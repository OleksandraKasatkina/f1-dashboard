...
def _fetch_standings(year):
    """
    Pull standings from the Jolpica REST API (Ergast-compatible, free, no key needed).
    Returns (drivers_list, constructors_list, error_message).
    """
...
def schedule_view(request):
    ...
    events, error_message = [], None
    try:
        races = _get_schedule(year)
...
def standings_view(request):
    ...
    drivers, constructors, error_message = _fetch_standings(year)
...
def results_view(request):
    ...
    race_results, quali_results, event_name, error_message = [], [], None, None
    try:
        races = _get_schedule(year)
        past = [r for _, r in races.iterrows() if r['EventDate'].date() <= date.today()]
        if not past:
            error_message = "No completed races yet this season."
        else:
            latest = past[-1]
    ...
    return render(request, 'dashboard/results.html', {
        'race_results': race_results, 'quali_results': quali_results,
...
def compare_view(request):
    ...
    chart_data, error_message = None, None
    race_options = []
    try:
        races = _get_schedule(year)
        for _, row in races.iterrows():
    ...
    if round_num and driver1 and driver2:
        try:
            ...
            if not d1['data'] and not d2['data']:
                error_message = "No lap data found. Check driver codes (e.g. VER, HAM, NOR)."
            else:
                chart_data = json.dumps([d1, d2])
        except Exception as exc:
            error_message = f"Could not load lap data: {exc}"

    return render(request, 'dashboard/compare.html', {
        'race_options': race_options, 'chart_data': chart_data,
        'selected_round': round_num, 'driver1': driver1, 'driver2': driver2,
        'year': year, 'available_years': AVAILABLE_YEARS,
        'error_message': error_message,
    })
...
def drivers_view(request):
    ...
    teams = sorted(set(d['team'] for d in DRIVERS_2026))
    drivers = [d for d in DRIVERS_2026 if not team_filter or d['team'] == team_filter]
    return render(request, 'dashboard/drivers.html', {
        'drivers': drivers, 'teams': teams, 'selected_team': team_filter,
    })

def driver_detail_view(request, code):
    driver = DRIVER_MAP.get(code.upper())
    if not driver:
...
def constructors_view(request):
    return render(request, 'dashboard/constructors.html', {'constructors': CONSTRUCTORS_2026})
...
def history_view(request):
    return render(request, 'dashboard/history.html', {
        'legends': LEGENDS, 'eras': HISTORY_ERAS, 'glossary': GLOSSARY,
    })


# ── Quiz ──────────────────────────────────────────────────────────────────────
# FIX: No Django sessions needed — correct answers are encoded in a hidden form
# field using base64, so python manage.py migrate is NOT required for the quiz.
...
def quiz_view(request):
    ...
    return render(request, 'dashboard/quiz.html', {
        'questions': questions, 'encoded_answers': encoded,
    })
...
def quiz_check_view(request):
    ...
    fact_map = {q['q']: q['fact'] for q in QUIZ_QUESTIONS}
    score, results = 0, []
    for q_text, correct_answer in answer_map.items():
        ...
        if correct:
            score += 1
        results.append({
            'question': q_text, 'user_answer': user_answer or '(no answer)',
            'correct_answer': correct_answer, 'correct': correct,
            'fact': fact_map.get(q_text, ''),
        })

    return render(request, 'dashboard/quiz_results.html', {
        'score': score, 'total': len(results), 'results': results,
    })